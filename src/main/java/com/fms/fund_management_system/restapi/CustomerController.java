...
public class CustomerController {
    ...
    public ResponseEntity<Page<CustomerModel>> fetchAll(@RequestParam(required = false) String customerName,
        ...
        return ResponseEntity.ok(customerService.getAllCustomers(customerName,status,startDate,endDate, search, pageable).map(CustomerModel::new));
    }

    @GetMapping(value = "/{customerId}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<CustomerModel> fetch(@PathVariable(value = "customerId") final Long customerId) {
        return ResponseEntity.ok(new CustomerModel(customerService.getCustomer(customerId)));
    }
